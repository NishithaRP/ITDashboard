// ============================================================
// IT INVENTORY MODULE — Supabase version
// ============================================================
...
async function inventoryRender(location) {
  ...
  if (el) el.innerHTML = invHTML(location, items) + invModal();
}
...
function invHTML(location, items) {
  ...
  return `<div>
    ...
    <div class="card" style="padding:0;"><div class="table-wrap"><table>
      <thead><tr><th>Employee</th><th>Type</th><th>Brand/Model</th><th>Serial #</th><th>Storage</th><th>RAM</th><th>Mouse</th><th>Actions</th></tr></thead>
...
function invRows(items) {
  if (items.length === 0) return `<tr><td colspan="8"><div class="empty-state"><div class="icon">💻</div><h3>No devices yet</h3></div></td></tr>`;
  return items.map(item => `<tr>
    ...
    <td>${item.storageType} ${item.storageSize}GB${item.extraHdd?`<br><span class="badge badge-yellow" style="font-size:10px;">+HDD ${item.extraHddSize}GB</span>`:''}</td>
    <td>${item.ram} GB</td>
  ...
  return `<div id="inv-modal" class="modal-overlay" style="display:none;">
    <div class="modal" style="max-width:780px;">
      ...
      <div class="modal-body"><div class="form-grid">
        ...
        <div class="form-group"><label>Serial Number</label><input id="inv-serial" type="text" placeholder="Device serial #"/></div>
...
function invTogglePCFields() { document.getElementById('inv-pc-fields').style.display = document.getElementById('inv-type').value === 'PC' ? 'grid' : 'none'; }
...
async function invSave() {
  ...
  const item = { id: invEditId||genId(), employee, deviceType,
